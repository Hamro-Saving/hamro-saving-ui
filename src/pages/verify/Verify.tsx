...
export default function Verify() {
  ...
  const { data: pendingDeposits } = useQuery({
    ...
    queryFn: () => savingsApi.getDeposits({ groupId: user?.groupId, isVerified: false }),
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">Review and verify pending deposits and payments</p>
      </div>

      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">Pending Deposits</h2>
          <span className="bg-amber-100 text-amber-700 text-xs font-semibold px-2.5 py-0.5 rounded-full">{pendingDeposits?.length ?? 0} pending</span>
        </div>
        <div className="divide-y divide-gray-50">
          {(pendingDeposits ?? []).map(d => (
            <div key={d.id} className="px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">{d.type} · Submitted {formatDate(d.createdAt)}</p>
          ...
          {!pendingDeposits?.length && (
            <div className="px-5 py-10 text-center">
              <svg className="w-10 h-10 text-emerald-200 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-sm text-gray-400">All caught up! No pending deposits.</p>
            </div>
          )}
        </div>
      </div>
