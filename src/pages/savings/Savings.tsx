...
import { formatCurrency, formatDate, BS_MONTHS, bsMonthName, currentBsDate, todayIso } from '../../utils/format';
...
function getFormErrors(form: { type: DepositType; amount: string; notes: string }) {
  const errors: Record<string, string> = {};
  const amount = Number(form.amount);
  if (!form.amount || isNaN(amount) || amount <= 0) {
    errors.amount = 'Amount is required.';
  } else if (form.type === 'MonthlyDeposit' && !MONTHLY_SAVING_AMOUNTS.includes(amount)) {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-gray-600 font-medium">Deposit Type</label>
                <select value={form.type} onChange={e => handleTypeChange(e.target.value as DepositType)} className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                  {DEPOSIT_TYPES.map(t => <option key={t} value={t}>{t.replace(/([A-Z])/g, ' $1').trim()}</option>)}
                </select>
              </div>
              ...
              <div>
                ...
                {form.type === 'MonthlyDeposit' ? (
                  ...
                  <input type="number" value={form.amount} onChange={e => setForm(f => ({...f, amount: e.target.value}))} className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Enter amount" />
                )}
                {formErrors.amount && <p className="mt-1 text-xs text-red-500">{formErrors.amount}</p>}
              </div>

              <div>
                <label className="text-xs text-gray-600 font-medium">Deposited For (BS)</label>
                <div className="grid grid-cols-2 gap-3 mt-1">
                  <select
                    value={form.depositMonth}
                    onChange={e => setForm(f => ({...f, depositMonth: Number(e.target.value)}))}
                  ...
                  <input
                    ...
                    onChange={e => setForm(f => ({...f, depositYear: Number(e.target.value)}))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    placeholder="2082"
                  />
                </div>
              ...
              <div>
                <label className="text-xs text-gray-600 font-medium">
                  Remarks {form.type === 'Other' ? <span className="text-red-500">*</span> : <span className="text-gray-400">(optional)</span>}
                </label>
                <textarea
                  value={form.notes}
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={handleSubmit} disabled={addMutation.isPending} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-60">
          ...
          <tbody className="divide-y divide-gray-50">
            ...
            {(deposits ?? []).map(d => (
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="px-5 py-3.5 text-gray-600">{bsMonthName(d.depositMonth)} {d.depositYear}</td>
                <td className="px-5 py-3.5"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${typeColors[d.type]}`}>{d.type.replace(/([A-Z])/g, ' $1').trim()}</span></td>
